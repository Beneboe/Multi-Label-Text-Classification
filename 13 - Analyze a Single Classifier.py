...
class_names = keras_type_names + random_type_names

predictions = (mt.load_prediction(CLASS, name) for name in class_names)
confusions = (mt.get_confusion(yi_predict) for yi_predict in predictions)
metrics = (mt.all_metrics(yi_predict, yi_expected) for yi_predict in predictions)
...
metric_comparison = pd.DataFrame(
    list(metrics),
    index=pd.Index([name for name in class_names]))
